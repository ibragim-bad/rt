...
int kiss_window_draw(kiss_window *window, SDL_Renderer *renderer)
{
	...
	if (window->decorate)
		kiss_decorate(renderer, &window->rect, kiss_black, kiss_edge);
	return 1;
}
	...
	SDL_Renderer *renderer)
{
	...
	kiss_decorate(renderer, &progressbar->rect, kiss_black, kiss_edge);
...
int kiss_entry_draw(kiss_entry *entry, SDL_Renderer *renderer)
{
	...
	color = kiss_black;
	if (entry->active) color = kiss_blue;
	if (entry->decorate)
...
int kiss_textbox_draw(kiss_textbox *textbox, SDL_Renderer *renderer)
{
	...
	if (textbox->decorate)
		kiss_decorate(renderer, &textbox->rect, kiss_black,
			kiss_edge);
